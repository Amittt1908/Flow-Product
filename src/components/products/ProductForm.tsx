...
export const ProductForm: React.FC<ProductFormProps> = ({ productId, onClose }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = `${t('products.productName')} ${t('products.required')}`;
    }

    if (!formData.price.trim()) {
      newErrors.price = `${t('products.price')} ${t('products.required')}`;
    } else if (isNaN(Number(formData.price)) || Number(formData.price) <= 0) {
      newErrors.price = `${t('products.price')} ${t('products.mustBePositive')}`;
    }

    if (!formData.quantity.trim()) {
      newErrors.quantity = `${t('products.quantity')} ${t('products.required')}`;
    } else if (isNaN(Number(formData.quantity)) || Number(formData.quantity) < 0) {
      newErrors.quantity = `${t('products.quantity')} ${t('products.mustBeNonNegative')}`;
    }

    if (!formData.category.trim()) {
      newErrors.category = `${t('products.category')} ${t('products.required')}`;
    }

    if (!formData.description.trim()) {
      newErrors.description = `${t('products.description')} ${t('products.required')}`;
    }
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {isEditing ? t('products.updateInfo') : t('products.fillDetails')}
              </p>
            </div>
          </div>
          <button
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <span>{isEditing ? t('products.updateProduct') : t('products.addProduct')}</span>
                </>
              )}
            </button>
          </div>
