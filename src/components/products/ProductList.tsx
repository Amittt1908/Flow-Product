...
export const ProductList: React.FC = () => {
  ...
  const getStockStatus = (quantity: number) => {
    if (quantity > 10) return { label: t('products.inStock'), color: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' };
    if (quantity > 5) return { label: t('products.lowStock'), color: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400' };
    return { label: t('products.veryLow'), color: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400' };
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        >
          ...
          <span className="font-semibold">{t('products.addProduct')}</span>
        </button>
      </div>
      ...
      {filteredProducts.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
            {searchTerm || selectedCategory ? t('products.noProductsFound') : t('dashboard.noProducts')}
          </h3>
          ...
          >
            <Plus className="w-5 h-5" />
            <span className="font-semibold">{t('products.addFirstProduct')}</span>
          </button>
        </div>
      )}
